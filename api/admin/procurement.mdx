...
<RequestExample>
```bash GET /x402/runtime-spend-limit
...
<Note>Requires admin authentication via `x-admin-key` header.</Note>

<RequestExample>
```bash POST /x402/runtime-spend-limit
...
<ParamField body="maxUsdc" type="string">
...
<RequestExample>
```bash POST /x402/runtime-spend-limit (clear)
curl -X POST https://api.example.com/x402/runtime-spend-limit \
  -H "Content-Type: application/json" \
  -H "x-admin-key: your-admin-key" \
  -d '{
